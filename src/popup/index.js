	...
	downloadButton.addEventListener('click', function () {
		...
		chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
			if (tabs && tabs[0] && tabs[0].id) {
				...
				// Don't need to execute the script separately as it's injected via manifest
				// Just send a message to trigger the download
				chrome.tabs.sendMessage(tabId, { type: 'TRIGGER_DOWNLOAD' }, response => {
