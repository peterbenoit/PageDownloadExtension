import { RESOURCE_TYPES, DEFAULT_SETTINGS } from './constants.js';
import { proxyConsole, updateFileStatus, blobToDataURL, downloadURL, getZipFilename } from './utils.js';
import { modifyHTML } from './html-modifier.js';
import JSZip from 'jszip';

// The main function that processes page data and creates the ZIP archive
		...
		chrome.storage.sync.get(DEFAULT_SETTINGS, function (settings) {
			...
			(async function processResources() {
				...
				for (const res of resources) {
					try {
						let type = RESOURCE_TYPES[res.type];
						...
						// Check if it's a font file based on extension or path
